...
本文定义 `model` 模块在重写方案中需要读取的配置项。`PolitDeck` 只有一份总配置文件，模型配置只是该 YAML 中的 `model` 配置段，不独立维护属于 `model` 模块的 YAML 文件。
...
该路径应由全局 `polit/config` 模块通过 `PolitHome` 和 `PolitConfigPath` 统一解析：`PolitHome` 默认是 `~/.politdeck`，`PolitConfigPath` 默认是 `~/.politdeck/politdeck.yaml`。全局 config 模块负责读取总配置文件并把其中的 `model` 段传给 `model` 模块；`model` 模块只校验和消费该配置段，不直接读取 YAML 文件、散落的环境变量、临时 CLI 参数、用户目录中的其他配置文件或运行时全局状态。
